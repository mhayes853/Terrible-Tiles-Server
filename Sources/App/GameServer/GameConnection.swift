...
struct GameConnection {
    fileprivate let connectionActor: GameConnectionActor
    
    init(gameActor: GameConnectionActor) {
...
extension GameConnection {
    fileprivate func runGameLoop() async throws {
        ...
        let spawnItemsLoop = Task { try await self.runGameActionLoop(interval: 10, loopfn: self.connectionActor.spawnNewItems) }
        let dropTileLoop = Task { try await self.runGameActionLoop(interval: 1, loopfn: self.connectionActor.dropRandomTile) }
        let attackPlayerLoop = Task { try await self.runGameActionLoop(interval: 5, loopfn: self.connectionActor.attackPlayer) }
        
        try await self.runGameActionLoop(interval: 0.3, loopfn: self.connectionActor.processPlayerInputs)
    ...
    private func runGameActionLoop(interval: TimeInterval, loopfn: () async throws -> Void) async throws {
